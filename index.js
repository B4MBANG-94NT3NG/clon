...
require('./Yaemiko.js')
nocache('../Yaemiko.js', module => console.log(color('[ CHANGE ]', 'green'), color(`'${module}'`, 'green'), 'Updated'))
require('./index.js')
...
if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +6287732200388")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Silakan Masukan Nomer WhatsApp Bot nya 🐦\nFor example: +6287732200388 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
	        ...
	        console.log(color(`\n${themeemoji} YT CHANNEL: BTR CHANNEL`,'magenta'))
            console.log(color(`${themeemoji} GITHUB: NO-GITHUB `,'magenta'))
            console.log(color(`${themeemoji} INSTAGRAM: @someone.ex_ `,'magenta'))
...
require('./Yaemiko')(XeonBotInc, m, chatUpdate, store)
} catch (err) {
console.log(err)}})
